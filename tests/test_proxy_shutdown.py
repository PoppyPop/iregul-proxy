...
async def test_shutdown_without_active_connections(
    ...
    assert not await proxy_server.downstream_handler.is_connected()
...
async def test_shutdown_with_active_connection(
    ...
    try:
        ...
        assert await proxy_server.downstream_handler.is_connected()
...
async def test_shutdown_rejects_multiple_active_connections(
    proxy_server: ProxyServer,
    mock_upstream: MockUpstreamServer,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that proxy accepts only one active downstream client.

    This test verifies that additional downstream clients are rejected while one
    active connection already exists, and shutdown remains graceful.
    ...
    primary_writer: asyncio.StreamWriter | None = None
    secondary_reader: asyncio.StreamReader | None = None
    secondary_writer: asyncio.StreamWriter | None = None

    try:
        _, primary_writer = await asyncio.open_connection(
            proxy_server.proxy_host, proxy_server.proxy_port
        )
        await asyncio.sleep(0.2)
        assert await proxy_server.downstream_handler.is_connected()

        secondary_reader, secondary_writer = await asyncio.open_connection(
            proxy_server.proxy_host, proxy_server.proxy_port
        )
        await asyncio.sleep(0.2)
        assert await proxy_server.downstream_handler.is_connected()

        secondary_data = await asyncio.wait_for(secondary_reader.read(1), timeout=1.0)
        assert secondary_data == b""

        test_message = b'{"test": "message"}'
        primary_writer.write(test_message)
        ...
        try:
            ...
            pytest.fail("Shutdown timed out with active connection")
        ...
        if primary_writer is not None:
            try:
                ...
                await primary_writer.wait_closed()
            except Exception:
                pass
        if secondary_writer is not None:
            try:
                secondary_writer.close()
                await secondary_writer.wait_closed()
            except Exception:
                pass
